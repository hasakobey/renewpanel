...
import sys,hashlib
r=Path(__file__).resolve().parent/'kart_sync';r.mkdir(exist_ok=True)
c=ssh();s=c.open_sftp();base='/var/www/kart.renewpanel.xyz/'
names=['kart-interactive-v4.js','index.html','kart-console-v1.js','kart-console-v1.css','mail-studio-v1.js','mail-studio-v1.css']
...
try:
 if sys.argv[1]=='fetch':
  ...
  backup='/var/backups/kart-sync-'+datetime.now().strftime('%Y%m%d-%H%M%S')
