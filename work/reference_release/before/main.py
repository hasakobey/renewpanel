...
import tempfile, shutil, os, json, re, zipfile
...
from .form_finance import router as form_finance_router
from .notification_service import notify_stock_created,start_worker,notify_transaction
...
from .db import init_db, connect, now, log, backup_db, backup_full, BASE
from .calc import sale_calc, stock_calc, get_settings
from .stock_history import capture_stock_month, load_stock_month, stock_as_of, valid_month
...
app.include_router(form_finance_router)
...
def startup():
    init_db();ensure_security_tables();ensure_default_admin();seed_from_master_if_empty();start_worker()

@app.get("/",response_class=HTMLResponse)
def home(): return (STATIC/"index.html").read_text(encoding="utf-8")

@app.get("/firebase-messaging-sw.js")
def firebase_messaging_worker():
    return FileResponse(STATIC/"firebase-messaging-sw.js",media_type="application/javascript",headers={"Service-Worker-Allowed":"/"})

@app.get("/renew-push/firebase-messaging-sw.js")
def renew_push_messaging_worker():
    return FileResponse(STATIC/"firebase-messaging-sw.js",media_type="application/javascript",headers={"Service-Worker-Allowed":"/renew-push/"})
...
def preview_sale(x:SaleIn):
    """Use the same calculation engine as the saved sale; never writes data."""
    _require_sale_fields(x)
    return {"ok":True,"calc":sale_calc(x.model_dump())}



@app.get("/api/vehicle-sources")
def vehicle_sources(month:str|None=None):
    with connect() as con:
        if month:
            if not valid_month(month):raise HTTPException(400,"Geçersiz çalışma dönemi.")
            period_rows,_=load_stock_month(con,month)
            stocks=[{k:r.get(k) for k in ("id","plate","brand","model","version","model_year","purchase_date")} for r in period_rows]
            stocks.sort(key=lambda r:(str(r.get("brand") or ""),str(r.get("model") or ""),str(r.get("plate") or "")))
        else:
        ...
        if month:
            sales=[dict(r) for r in con.execute("SELECT id,plate,vehicle_info,model_year,sale_date FROM sales WHERE substr(sale_date,1,7)=? ORDER BY sale_date DESC,id DESC",(month,)).fetchall()]
            acquisitions=[dict(r) for r in con.execute("SELECT id,plate,brand,model,version,model_year,purchase_date,acquired_by FROM acquisitions WHERE substr(purchase_date,1,7)=? ORDER BY purchase_date DESC,id DESC",(month,)).fetchall()]
        else:
            sales=[dict(r) for r in con.execute("SELECT id,plate,vehicle_info,model_year,sale_date FROM sales ORDER BY sale_date DESC,id DESC").fetchall()]
            acquisitions=[dict(r) for r in con.execute("SELECT id,plate,brand,model,version,model_year,purchase_date,acquired_by FROM acquisitions ORDER BY purchase_date DESC,id DESC").fetchall()]
    for row in stocks+sales+acquisitions:
        pkey=re.sub(r"[^A-Za-z0-9_-]","",str(row.get("plate") or "").upper());meta_path=BASE/"vehicle_media"/pkey/"meta.json";photo_count=0
        try:
            meta=json.loads(meta_path.read_text(encoding="utf-8"));photo_count=sum(1 for value in meta.get("slots",{}).values() if (meta_path.parent/str(value)).is_file())
        except Exception:
            folder=meta_path.parent/"listing";photo_count=sum(1 for f in folder.iterdir() if f.is_file() and f.suffix.lower() in {".jpg",".jpeg",".png",".webp"}) if folder.exists() else 0
        row["photo_count"]=min(photo_count,25)
    return {"stocks":stocks,"sales":sales,"acquisitions":acquisitions,"month":month}

def _studio_plate(value:str)->str:
    plate=re.sub(r"[^A-Za-z0-9_-]","",str(value or "").upper())
    if not plate: raise HTTPException(400,"Araç seçimi zorunlu.")
    return plate

def _studio_root(plate:str)->Path:
    root=BASE/"vehicle_media"/_studio_plate(plate);(root/"listing").mkdir(parents=True,exist_ok=True);return root

def _studio_meta(root:Path):
    path=root/"meta.json"
    try:return json.loads(path.read_text(encoding="utf-8")) if path.exists() else {"cover":"","order":[],"labels":{}}
    except Exception:return {"cover":"","order":[],"labels":{}}

def _studio_save_meta(root:Path,data:dict):(root/"meta.json").write_text(json.dumps(data,ensure_ascii=False,indent=2),encoding="utf-8")

def _studio_slots(root:Path):
    meta=_studio_meta(root);slots={};raw=meta.get("slots",{})
    for key,value in raw.items():
        try:n=int(key)
        except Exception:continue
        f=root/str(value)
        if 1<=n<=25 and f.is_file():slots[n]=f
    # Backward compatibility: place earlier listing photos into the first free slots.
    ordered=[]
    for key in meta.get("order",[]):
        f=root/str(key)
        if str(key).startswith("listing/") and f.is_file() and f not in ordered:ordered.append(f)
    for f in sorted((root/"listing").iterdir(),key=lambda x:x.stat().st_mtime):
        if f.is_file() and f.suffix.lower() in {".jpg",".jpeg",".png",".webp"} and f not in ordered:ordered.append(f)
    used=set(slots.values())
    for f in ordered:
        if f in used:continue
        free=next((n for n in range(1,26) if n not in slots),None)
        if free is None:break
        slots[free]=f;used.add(f)
    meta["slots"]={str(n):str(f.relative_to(root)).replace("\\","/") for n,f in slots.items()};meta["order"]=[meta["slots"][str(n)] for n in sorted(slots)];_studio_save_meta(root,meta)
    return slots,meta

@app.get("/api/media-studio/{plate}")
def media_studio_state(plate:str):
    p=_studio_plate(plate);root=_studio_root(p);slots,meta=_studio_slots(root)
    return {"plate":p,"vehicle_id":meta.get("vehicle_id"),"count":len(slots),"slots":[{"slot":n,"name":f.name,"url":f"/api/media/{p}/file/listing/{f.name}"} for n,f in sorted(slots.items())]}

@app.post("/api/media-studio/upload")
async def media_studio_upload(plate:str=Form(...),files:list[UploadFile]=File(...),slot:int|None=Form(None),vehicle_id:int|None=Form(None)):
    p=_studio_plate(plate);root=_studio_root(p);folder=root/"listing";allowed={".jpg",".jpeg",".png",".webp"}
    slots,meta=_studio_slots(root);valid=[x for x in files if Path(x.filename or "").suffix.lower() in allowed]
    if slot is not None and (slot<1 or slot>25):raise HTTPException(400,"Fotoğraf numarası 1–25 arasında olmalıdır.")
    free=[n for n in range(1,26) if n not in slots];targets=[slot] if slot is not None else free[:len(valid)]
    if len(valid)>len(targets):raise HTTPException(400,"25 fotoğraf sınırına ulaşıldı.")
    saved=[]
    for item,target_slot in zip(valid,targets):
        old=slots.get(target_slot)
        if old and old.exists():old.unlink()
        ext=Path(item.filename or "foto.jpg").suffix.lower();target=folder/f"{target_slot:02d}{ext}"
        for candidate in folder.glob(f"{target_slot:02d}.*"):
            if candidate!=target and candidate.is_file():candidate.unlink()
        with target.open("wb") as out:shutil.copyfileobj(item.file,out)
        slots[target_slot]=target;saved.append({"slot":target_slot,"name":target.name})
    meta["slots"]={str(n):str(f.relative_to(root)).replace("\\","/") for n,f in slots.items()};meta["order"]=[meta["slots"][str(n)] for n in sorted(slots)]
    if vehicle_id:meta["vehicle_id"]=vehicle_id
    _studio_save_meta(root,meta);log("media",p,"UPLOAD",f"İlan stüdyosu: {len(saved)} fotoğraf")
    return {"ok":True,"count":len(saved),"total":len(slots),"saved":saved}

@app.post("/api/media-studio/{plate}/swap")
def media_studio_swap(plate:str,payload:dict):
    a=int(payload.get("a") or 0);b=int(payload.get("b") or 0)
    if not (1<=a<=25 and 1<=b<=25):raise HTTPException(400,"Numaralar 1–25 arasında olmalıdır.")
    root=_studio_root(plate);slots,meta=_studio_slots(root);fa,fb=slots.get(a),slots.get(b)
    if not fa or not fb:raise HTTPException(400,"Yer değiştirmek için iki dolu kare seçin.")
    slots[a],slots[b]=fb,fa;meta["slots"]={str(n):str(f.relative_to(root)).replace("\\","/") for n,f in slots.items()};meta["order"]=[meta["slots"][str(n)] for n in sorted(slots)];_studio_save_meta(root,meta)
    return {"ok":True}

@app.delete("/api/media-studio/{plate}/slot/{slot}")
def media_studio_delete_slot(plate:str,slot:int):
    if not 1<=slot<=25:raise HTTPException(400,"Geçersiz numara.")
    root=_studio_root(plate);slots,meta=_studio_slots(root);f=slots.pop(slot,None)
    if f and f.exists():f.unlink()
    meta["slots"]={str(n):str(x.relative_to(root)).replace("\\","/") for n,x in slots.items()};meta["order"]=[meta["slots"][str(n)] for n in sorted(slots)];_studio_save_meta(root,meta);return {"ok":True}

@app.get("/api/media-studio/{plate}/download")
def media_studio_download(plate:str,slot_numbers:str=""):
    p=_studio_plate(plate);root=_studio_root(p);slots,_=_studio_slots(root)
    requested={int(x) for x in slot_numbers.split(",") if x.strip().isdigit() and 1<=int(x)<=25}
    if requested:slots={n:f for n,f in slots.items() if n in requested}
    if not slots:raise HTTPException(404,"İndirilecek fotoğraf bulunamadı.")
    out=root/f"{p} fotoğrafları.zip"
    with zipfile.ZipFile(out,"w",zipfile.ZIP_DEFLATED) as archive:
        for number,f in sorted(slots.items()):archive.write(f,f"{number:02d}{f.suffix.lower()}")
...
def dashboard(month:str):
    ...
    pm=prev_month(month)
    with connect() as con:
        stocks,stock_period=load_stock_month(con,month)
    ...
    stock_date=stock_as_of(month)
    sc=[stock_calc(x,stock_date) for x in stocks];cc=[sale_calc(x) for x in sales];pc=[sale_calc(x) for x in prev]
...
@app.get("/api/stocks")
def stocks(month:str|None=None):
    with connect() as con:
        if month:
            rows,meta=load_stock_month(con,month)
        else:
            rows=[dict(r) for r in con.execute("SELECT * FROM vehicles WHERE status='STOCK' ORDER BY purchase_date,id").fetchall()]
            meta={"mode":"live"}
    as_of=stock_as_of(month) if month else None
    historical=bool(month and month<datetime.now().strftime("%Y-%m"))
    return [{**r,"_stock_period":meta,"_historical":historical,"calc":stock_calc(r,as_of)} for r in rows]

def _sync_current_stock_snapshot(con, source="manual"):
    """Keep the open month's stock view in sync with manual stock operations."""
    current_month=datetime.now().strftime("%Y-%m")
    capture_stock_month(con,current_month,source=source)
...
def add_stock(x:StockIn):
    ...
    with connect() as con:
        ...
        _sync_current_stock_snapshot(con,"manual_create" if act=="CREATE" else "manual_reactivate")
    log("vehicle",vid,act,x.plate)
    if act=="CREATE":
        with connect() as con: vehicle=dict(con.execute("SELECT * FROM vehicles WHERE id=?",(vid,)).fetchone())
        try: notify_stock_created(vehicle)
        except Exception: pass
...
def update_stock(vid:int,x:StockIn):
    ...
    with connect() as con:
        ...
        _sync_current_stock_snapshot(con,"manual_update")
    log("vehicle",vid,"UPDATE",x.plate);return {"ok":True}

@app.delete("/api/stocks/{vid}")
def delete_stock(vid:int):
    with connect() as con:r=con.execute("SELECT plate FROM vehicles WHERE id=?",(vid,)).fetchone();con.execute("DELETE FROM vehicles WHERE id=?",(vid,));_sync_current_stock_snapshot(con,"manual_delete")
...
def sell_stock(vid:int,x:SaleIn):
    ...
    with connect() as con:
        ...
        _sync_current_stock_snapshot(con,"manual_sale")
    log("vehicle",vid,"SELL",x.plate);log("sale",cur.lastrowid,"CREATE",x.plate)
    notify_transaction('sale_created',cur.lastrowid,vals)
    return {"ok":True}
...
def add_sale(x:SaleIn):
    ...
    log("sale",sid,action,x.plate)
    if action=='CREATE':notify_transaction('sale_created',sid,vals)
...
def add_acquisition(x:AcquisitionIn):
    ...
    _sync_acquisition_expertise(x.purchase_date[:7],x.acquired_by);log("acquisition",aid,"CREATE",f"{x.plate} • {x.acquired_by}")
    notify_transaction('acquisition_created',aid,vals)
...
async def preview_stock(file:UploadFile=File(...)):
    ...
    try:
        ...
        with connect() as con:
            existing={_norm_choice(r["plate"]):dict(r) for r in con.execute("SELECT * FROM vehicles WHERE status='STOCK'").fetchall()}
        fields=("model_year","km","brand","model","version","color","fuel","transmission","purchase_date","purchase_type","purchase_price","list_price","target_profit")
        new_count=changed_count=duplicate_count=0
        seen=set()
        for row in rows:
            key=_norm_choice(row.get("plate")); old=existing.get(key)
            if key in seen: duplicate_count+=1; row["_diff_status"]="duplicate"
            elif not old: new_count+=1; row["_diff_status"]="new"
            elif any(_norm_choice(old.get(k))!=_norm_choice(row.get(k)) for k in fields): changed_count+=1; row["_diff_status"]="changed"
            else: duplicate_count+=1; row["_diff_status"]="same"
            seen.add(key)
        return {"rows":rows,"file_name":file.filename,"count":len(rows),"diff":{"new":new_count,"changed":changed_count,"duplicate":duplicate_count}}
    except Exception as e:
...
def apply_stock(payload:dict):
    ...
    if not valid_month(month): raise HTTPException(400,"Stok aktarımı için çalışma dönemi seçilmelidir.")
    backup_db()
    ...
    if month==current_month:
        if mode=="replace":
            with connect() as con: con.execute("DELETE FROM vehicles WHERE status='STOCK'")
        count=0
        for r in rows:
            add_stock(StockIn(**r));count+=1
        with connect() as con: capture_stock_month(con,month,source=f"excel_{mode}")
    else:
        with connect() as con:
            base=[] if mode=="replace" else load_stock_month(con,month)[0]
            by_plate={_norm_choice(r.get("plate")):dict(r) for r in base if r.get("plate")}
            next_id=max([int(r.get("id") or 0) for r in base]+[0])+1
            for row in rows:
                item=dict(row);key=_norm_choice(item.get("plate"));old=by_plate.get(key,{})
                item={**old,**item,"id":old.get("id") or next_id,"status":"STOCK","updated_at":now(),"created_at":old.get("created_at") or now()}
                if not old.get("id"):next_id+=1
                by_plate[key]=item
            count=capture_stock_month(con,month,list(by_plate.values()),source=f"historical_excel_{mode}")
    log("stock_import",month,mode.upper(),f"{count} araç")
    return {"ok":True,"count":count,"mode":mode,"month":month}
...
async def preview_sales(month:str=Form(...),file:UploadFile=File(...)):
    ...
    try:
        ...
        with connect() as con:
            existing={( _norm_choice(r["plate"]),str(r["sale_date"] or "")[:10]):dict(r) for r in con.execute("SELECT * FROM sales WHERE substr(sale_date,1,7)=?",(month,)).fetchall()}
        fields=("vehicle_info","model_year","purchase_type","purchase_price","purchase_date","sale_date","extra_expense","customer","sale_type","sale_price","consultant","insurance_income","credit_income","warranty_income")
        new_count=changed_count=duplicate_count=0
        seen=set()
        for row in rows:
            key=(_norm_choice(row.get("plate")),str(row.get("sale_date") or "")[:10]); old=existing.get(key)
            if key in seen: duplicate_count+=1; row["_diff_status"]="duplicate"
            elif not old: new_count+=1; row["_diff_status"]="new"
            elif any(_norm_choice(old.get(k))!=_norm_choice(row.get(k)) for k in fields): changed_count+=1; row["_diff_status"]="changed"
            else: duplicate_count+=1; row["_diff_status"]="same"
            seen.add(key)
        return {"rows":rows,"file_name":file.filename,"month":month,"count":len(rows),"diff":{"new":new_count,"changed":changed_count,"duplicate":duplicate_count}}
    except Exception as e:
...
def export_excel(month:str|None=None):
    try:
        ...
        period=(month or datetime.now().strftime("%Y-%m")).replace("-","_")
        return FileResponse(out,filename=f"RENEW_{period}_{datetime.now():%Y%m%d_%H%M}.xlsx",media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    except Exception as e:
...
@app.post("/api/backup")
def backup():
    db_copy=backup_db();full=backup_full();return {"ok":bool(full),"file":full.name if full else None,"database_file":db_copy.name if db_copy else None}

@app.get("/api/backups")
def backups():
    root=BASE/"backups";root.mkdir(parents=True,exist_ok=True)
    rows=[]
    files=list(root.glob("renew_*.db"))+list(root.glob("renew_full_*.tar.gz"))
    for p in sorted(files,key=lambda x:x.stat().st_mtime,reverse=True)[:100]:
...
def download_backup(name:str):
    ...
    if clean!=name or not clean.startswith("renew_") or not (clean.endswith(".db") or clean.endswith(".tar.gz")):raise HTTPException(400,"Geçersiz yedek adı")
