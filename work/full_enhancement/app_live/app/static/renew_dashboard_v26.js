...
function renderRenewAlerts(d){
 ...
 const items=d.management_alerts||[];
 box.innerHTML=items.length?items.map(x=>`<button class="dash-alert ${esc(x.level)}" onclick="openRenewAlert('${esc(x.target)}','${esc(x.filter||'')}')"><span>${x.level==='critical'?'!':x.level==='warning'?'△':'i'}</span><div><b>${esc(x.title)}</b><small>İncelemek için aç</small></div><strong>${renewNum(x.count)}</strong></button>`).join(''):'<div class="dash-alert success"><span>✓</span><div><b>Günlük kontroller temiz</b><small>Kritik kayıt bulunmuyor.</small></div></div>';
}
function openRenewAlert(target,filter){
 page(target);
 if(target==='stocks'){
  if(filter==='90_plus'){const x=document.getElementById('stockSks');if(x)x.value='90_plus'}
  renderStocks();
 }
 if(target==='photos'&&filter==='missing_media')setTimeout(()=>document.getElementById('mediaVehicleSelect')?.focus(),100);
}
...
renderDashboard=function(d){RENEW_DASH_DATA=d;_renewOriginalRenderDashboard(d);renderRenewAlerts(d);renderRenewTrend(d);renderRenewSourceValue(d)};
