...
PERMISSION_LABELS={
 ...
 ,"notifications.manage":"Bildirimleri Yönetme"
}
...
# Bildirim görüntüleme tüm rollerin, merkezi ayarlar ise yönetici ve müdürün kapsamındadır.
for _role in ROLE_DEFAULTS:
    if "notifications.view" not in ROLE_DEFAULTS[_role]: ROLE_DEFAULTS[_role].append("notifications.view")
for _role in ("administrator","manager"):
    if "notifications.manage" not in ROLE_DEFAULTS[_role]: ROLE_DEFAULTS[_role].append("notifications.manage")
...
def required_permission(path,method):
    ...
    if path.startswith("/api/notifications/settings") or path.startswith("/api/notifications/run-check") or path.startswith("/api/notifications/manual"):
