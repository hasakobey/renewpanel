...
def export_master(dest, month=None):
    ...
    if month:
        ...
        if len(month)!=7 or month[4]!="-" or not month[:4].isdigit() or not month[5:].isdigit() or not 1<=int(month[5:])<=12:
            raise ValueError(f"Geçersiz çalışma dönemi: {month!r}")
    with connect() as con:
        if month:
            _sales=[dict(r) for r in con.execute("SELECT * FROM sales WHERE substr(sale_date,1,7)=? ORDER BY sale_date,id",(month,)).fetchall()]
        else:
        ...
        # Expertise personnel definitions persist, but monthly operation counts do not.
        expertise_names=[r["consultant"] for r in con.execute("SELECT DISTINCT consultant FROM expertise ORDER BY consultant").fetchall()]
        expertise_month={r["consultant"]:dict(r) for r in con.execute("SELECT * FROM expertise WHERE month=?",(month,)).fetchall()}
        expertise=[{
            "consultant":name,
            "month":month,
            "done_count":int(expertise_month.get(name,{}).get("done_count",0) or 0),
            "converted_count":int(expertise_month.get(name,{}).get("converted_count",0) or 0)
    ...
    with ZipFile(TEMPLATE,"r") as zin, ZipFile(tmp,"w",ZIP_DEFLATED) as zout:
        ...
        # Formüller yeniden oluşturulduğu için eski calcChain dosyası bilinçli olarak
        # çıkarılır. Paket ilişkisinin ve content-type kaydının da kaldırılması gerekir;
        # aksi halde sıkı OpenXML okuyucuları eksik parça hatası verir.
        rel_path="xl/_rels/workbook.xml.rels"
        if rel_path in zin.namelist():
            rel_xml=zin.read(rel_path)
            rel_xml=re.sub(rb'<Relationship\b(?=[^>]*(?:Target="calcChain\.xml"|Type="[^"]*/calcChain"))[^>]*/>',b'',rel_xml)
            replacements[rel_path]=rel_xml
        content_path="[Content_Types].xml"
        if content_path in zin.namelist():
            content_xml=zin.read(content_path)
            content_xml=re.sub(rb'<Override\b(?=[^>]*PartName="/xl/calcChain\.xml")[^>]*/>',b'',content_xml)
            replacements[content_path]=content_xml
