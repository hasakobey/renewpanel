...
(()=>{function init(){window.__renewProbe=window.__renewProbe||[];window.__renewProbe.push({script:'renew-stitch-v1.js',readyState:document.readyState,rcWorkspace:!!document.querySelector('.rc-workspace'),rcHistory:!!document.getElementById('rcHistory'),rcCompareBody:!!document.getElementById('rcCompareBody'),rcMiniActions:!!document.querySelector('.rc-mini-actions'),t:Math.round(performance.now())});const card=document.getElementById('finansKartPanel'),tabs=document.getElementById('finansModSecici');if(!card||!tabs)return;document.body.classList.add('rst-v1');
